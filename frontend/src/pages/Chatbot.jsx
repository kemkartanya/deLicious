...
const Chatbot = () => {
  ...
  const createReceipe = async () => {
    try {
      ...
      const { data } = await axios.post("http://localhost:8000/api/v1/recipes", {
        values:textAreaValue + "User Preference: " + " favorite meats: " + userPref.favorite_meats + "cooking_skill_level" + userPref.cooking_skill_level + "cuisine_types: " + userPref.cuisine_types + "dietary_restrictions: " + userPref.dietary_restrictions + "spice_tolerance: " + userPref.spice_tolerance
      });
    ...
    <div className="flex flex-col overflow-hidden" style={{ height: "90vh" }}>
      <div className="m-4 p-4 flex" style={{ height: "75%" }}>
        <div className="chat chat-start w-1/2 m-2">
          <div className="chat-bubble bg-red-700">
          Delicious Chicken Curry
          </div>
        </div>
      ...
      <div className="suggestions flex items-center justify-center m-4">
        <button onClick={prevSuggestion} className="btn btn-circle mx-4 bg-red-700 text-white">❮</button>
        ...
        <button onClick={nextSuggestion} className="btn btn-circle mx-4 bg-red-700 text-white">❯</button>
      </div>
      <div className="px-10 py-5 flex-grow flex flex-col justify-end overflow-auto">
        <div className="flex ">
          ...
          <button className="btn mx-1 h-full bg-red-700 text-white" onClick={createReceipe}>Create My Recipe</button>
        </div>
      </div>
    </div>
  );
